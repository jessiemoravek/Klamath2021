...
airtemp <- airtemp[c(1:377),]
str(airtemp)
head(airtemp)
tail(airtemp)
#AND maximum temperature occurs about 4 days before maximum water temperature
#so I need to shift covariate data to the RIGHT by 4 days
#cut to fit our data (3 July through 24 July)
plot(LL3_daily$mean_temp)
plot(airtemp$TAVG)
ggplot() + 
  geom_point(data = LL3_daily, aes(x = day, y = mean_temp, color = "blue")) +
  geom_point(data = airtemp, aes(x = date, y = TAVG, color = "red")) +
  theme_classic() 
airtemp <- airtemp[c(32:53),] 

str(airtemp)

#Convert to time series
airtempts <- ts(airtemp$TAVG, start = c(3, 1), frequency = 1) #The sensor records 1 reading per day. This time series starts on July 3 
ts.plot(airtempts,main="Temperature",ylab = "Temperature (C)", xlab = "Day of July 2020") 

#Build the little c matrix, call it matrixc
matrixc <- matrix(nrow=1,ncol=22)
