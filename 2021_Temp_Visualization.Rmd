...
daily_means_long <- t(daily_means[,-c(1,28)])
dim(daily_means_long)

covariate <- t(daily_means[,c(28)])

matplot(t(covariate))
matplot(daily_means[,18])

```
#####Summer Averages
```{r}
creeks <- daily_means[c(31:91), c("APck","SPck","LSck","MayCk","FGck","GPck","ULck", "day")]
ponds <- daily_means[c(31:91), c("APck","SPck","LSck","MayCk","FGck","GPck","ULck")]

ggplot()+
  geom_boxplot(data = daily_means_long, aes(x = day, y = site))+
  theme_classic()
```
